...
void GUI::PrintMessage(string msg) const	//Prints a message on status bar: line 1
{
...
//	pWind->SetFont(18, BOLD , BY_NAME, "Arial");   
//	
//	//------------------Print Message On Multiple Lines------------------   
//
//	int charsPerLine = 106; //Pick number of characters per line
//
//	int noOfLines = ceil((msg.length() / charsPerLine)) + 1;  //No of Lines 
//
//	string* parsedMessage = new string[noOfLines];  //Dynamic String Array holds each line
//
//	int Y = WindHeight - (int) (StatusBarHeight/1.5);  //Height to print at on GUI
//
...
//
//			//--If character exists in msg (Prevent overload)
//			if(i*charsPerLine + j <= msg.length()){
//				
//				parsedMessage[i] += msg[i*charsPerLine + j]; //Add character to 
//														     //the string to be printed
//
//			} else
//				
//				parsedMessage[i] += " "; //Add empty space instead
//
//		}
//
//		pWind->DrawString(10, Y, parsedMessage[i]); //Print string
//		Y += 20; //Increase Y to print at for the next line
//	}	
//
//	parsedMessage = nullptr;	//Dynamic pointer deallocation
//	delete parsedMessage;
//}

void GUI::PrintMessage2(string msg) const	//Prints a message on status bar: line 2
{
...
void GUI::PrintMessage3(string msg) const	//Prints a message on status bar:line 3
{

	pWind->SetPen(RED);
	pWind->SetFont(18, BOLD, BY_NAME, "Arial");
	pWind->DrawString(10, WindHeight - (int)(StatusBarHeight /1.8), msg); 

}

void GUI::PrintMessage4(string msg) const	//Prints a message on status bar:line 4
{

	pWind->SetPen(DARKBLUE);
	pWind->SetFont(18, BOLD, BY_NAME, "Arial");
	pWind->DrawString(10, WindHeight - (int)(StatusBarHeight / 2.5), msg); 

}

void GUI::PrintMessage5(string msg) const	//Prints a message on status bar:line 5
{

	pWind->SetPen(DARKRED);
	pWind->SetFont(18, BOLD, BY_NAME, "Arial");
	pWind->DrawString(10, WindHeight - (int)(StatusBarHeight / 3.9), msg);
...
int GUI::getFileName() const
{
	int n;

	PrintMessage("Please select the required input file: (1)TC1, (2)TC2, (3)TC3, (4)TC4, (5)TC5, (6)TC6");
	string S = GetString();
	n = (atoi(S.c_str()) - 1);
